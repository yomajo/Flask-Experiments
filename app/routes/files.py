...
def upload_file():
    '''dropzone posts to same route. Fixing that is a pain in js or new plugin (flask-dropzone)'''
        ...
        for i, file in enumerate(files, start=1):
            ...
            if file and allowed_file(file.filename):
                ...
                if save_to_path.lower().endswith('.xlsx'):
                    val_from_wb = read_workbook(save_to_path)
                ...
                did_some_work = True
        db.session.commit()
        print(f'saved file: {filename} to db. Do I redirect?')
        msg = f'Successfully uploaded {files_uploaded_counter} files'
        return redirect(url_for('files_bp.upload_processing'))
        # return render_template('upload_processing.html', )
    
    # GET 
    accepted_exts = ', '.join(ALLOWED_EXTENSIONS)
    return render_template('upload_file.html', accepted_exts=accepted_exts)

@files_bp.route('/upload_processing')
